...
with open('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML-fullwn\important_wavenumbers.txt') as json_file:
    important_wavenumb = json.load(json_file)

# Select the important wavenumbers from the main dataframe
train_features = X[[wavenumber for wavenumber in X.columns if wavenumber in important_wavenumb]]
...
with open('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML-selected wavenumbers\classifier.pkl', 'rb') as fid:
...
cr = cr.iloc[0:]
cr.to_csv('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML-selected wavenumbers\classification_report_.csv')
