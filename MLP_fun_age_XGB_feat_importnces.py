# This program uses deep learning trainied with important wavenumbers identified by the 
# XGBoost to predict the age structure of of Anopheles funestus mosquitoes collected 
# from the wild 
...
# Function for plotting confusion matricies
...
# method learn't from `Deep Learning with Python` by François Chollet
...
def graph_history_averaged(combined_history):
    ...
    plt.tight_layout()
    plt.grid(False)
    plt.savefig("C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\Fold\Training_Folder\Averaged_graph.png", dpi = 500, bbox_inches="tight")
    plt.close()
...
# Find the average of all combined dictionaries 
...
# Dimension reduction with selected wavenumbers from XGboost (feature importances)

# The idea here is to reduce the dimensianality of a dataset consisting of a large number 
# of related variables while retaining as much variance in the data as possible. We will use wavenumbers selected using select from 
# XGBoost
...
with open('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML\important_wavenumbers.txt') as json_file:
    important_wavenumb = json.load(json_file)

# Select the important wavenumbers from the main dataframe

new_data = X[[wavenumber for wavenumber in X.columns if wavenumber in important_wavenumb]]
...
scl = StandardScaler()
scaler = scl.fit(X = new_data)
X_new = scaler.transform(X = new_data)

# transform X matrix and y list of labels as arrays
...
with open('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\Fold\Training_Folder\combined_history_dictionaries.txt', 'w') as outfile:
...
# %%
# Loading dataset for prediction/test data
...
# define matrix of features and vector of labels for test set
...
X_valid_new = scaler.transform(X = new_data_val)

# transform X and y matrices as arrays
age_valid = np.asarray(X_valid_new)
...
loaded_model = load_model("C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\Fold\Training_Folder_selected_wns\lCNN_0_3_Model.h5")
...
cr_1 = classification_report(np.argmax(y_validation, axis=-1), np.argmax(predictions, axis=-1))
print(cr_1)

# save classification report to disk 
cr = pd.read_fwf(io.StringIO(cr_1), header=0)
cr = cr.iloc[0:]
cr.to_csv('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\Fold\Training_Folder_selected_wns\classification_report.csv')
