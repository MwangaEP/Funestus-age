...
from sklearn.metrics import accuracy_score, roc_auc_score, roc_curve, confusion_matrix, classification_report, f1_score, recall_score, precision_score, precision_recall_fscore_support
...
models.append(('LR', LogisticRegressionCV(multi_class = 'ovr', cv = kf, max_iter = 3500, random_state = seed)))
...
for name, model in models:
    cv_results = cross_val_score(
        model, scaled_features, labels, cv = kf, scoring = scoring)
...
plt.figure(figsize = (6, 4))
sns.boxplot(x = results_df['Model'], y = results_df['Accuracy'], width = .4)
    ...
    for train_index, test_index in kf.split(scaled_features, labels):
        ...
        rsCV = RandomizedSearchCV(verbose = 1,
                    estimator = classifier, param_distributions = param_grid, n_iter = n_iter_search, 
                                scoring = scoring, cv = kf, refit = True)
        ...
        local_feat_impces = pd.DataFrame(classifier.feature_importances_,
                                         index = train_features.columns).sort_values(by = 0, ascending = False)
...
kf_results = pd.read_csv("C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML\crf_kfCV_record.csv")

# Accuracy distribution
crf_acc_distrib = kf_results["Accuracy"]
crf_acc_distrib.columns=["Accuracy"]
crf_acc_distrib.to_csv("C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML\crf_acc_distrib.csv", header=True, index=False)
crf_acc_distrib = pd.read_csv("C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML\crf_acc_distrib.csv")
...
rskf_results = pd.read_csv("C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML\crf_kfCV_record.csv")
...
plt.savefig(("C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML\_feature_impces_full_wn.png"), dpi = 500, bbox_inches="tight")
...
with open('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML\important_wavenumbers.txt', 'w') as outfile:
...
with open('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML-fullwn\classifier.pkl', 'rb') as fid:
...
cr = cr.iloc[0:]
cr.to_csv('C:\Mannu\Projects\Anophles Funestus Age Grading (WILD)\std_ML-fullwn\classification_report_.csv')
